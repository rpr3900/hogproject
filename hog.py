...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while num_rolls > 0:
        ...
        if dice_output == 1:
            num_one = True
        x =x + dice_output #unsure, may change to x += dice_output
        num_rolls =num_rolls - 1 #unsure, may change to num_rolls -= 1
    if num_one: #if there's a one return a one, this relates to SOW SAD
        return 1
    else: 
        return x
...
def piggy_points(score):
    ...
    """
    # BEGIN PROBLEM 2
    # basically, returning opponents score and returning the no. of points
    # by rolling 0 dice
...
def more_boar(player_score, opponent_score):
    ...
    if player_score < 10:
        play_left = 0
        play_sec = player_score
    else:
        while player_score >= 10:
            play_left, play_sec = (player_score // 10) % 10, player_score % 10
            player_score = player_score // 10
    if opponent_score < 10:
        opp_left = 0
        opp_sec = opponent_score
    else:
        while opponent_score >= 10:
            opp_left, opp_sec = (opponent_score // 10) % 10, opponent_score % 10
            opponent_score = opponent_score // 10
    if play_left < opponent_score and play_sec < opp_sec:
        return True
    return False
...
def piggypoints_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    if piggypoints_strategy(opponent_score) >= cutoff:
        return 0 
    else:
        return num_rolls 
